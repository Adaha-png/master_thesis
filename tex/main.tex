...
\begin{acronym}[ICANN]
    ...
    \acro  {xai}  [XAI]  {Explainable Artificial Intelligence}    % will not be listed, as it is not used
...
\section{Motivation}
The motivation behind this research comes from the challenge of interpreting and trusting \ac{rl} systems, particularly in \ac{marl} environments. RL models have achieved significant success in many applications, they often suffer from the "black box" problem, where their decision-making processes remain opaque. This lack of transparency could prove a significant risk in safety-critical applications such as autonomous driving. Without understanding why an agent makes a decision, deploying \ac{drl}-based systems in real-world scenarios becomes problematic.
...
\end{tikzpicture}
\caption{Diagram showing the PPO update algorithm}
\label{fig:rl_basics}
\end{center}
\end{figure}
...
We aim to construct a framework to describe the intent of the agents in the KAZ environment, and we aim to make it applicable to other environments as well. First we will go a bit more in depth on what type of explainablity we want to use as well as describe relevant methods already used, then we will attempt to integrate previous and own methods into a \ac{marl} policy we construct and train, and at the end we will discuss our findings.
...
Intrinsically explainable models have the already stated drawback of relying on careful model construction, which can be challenging and may compromise model performance. While these models offer transparency from the outset, they are often less flexible and may not achieve the same level of accuracy as more complex \ac{rl} models. Given that we aim to analyze and compare already trained \ac{rl} policies, we will mostly focus on post hoc explainability methods in this thesis.
